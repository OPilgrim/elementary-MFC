...
SimpleFactory*op= new SimpleFactory();
void Equation::creat()
{
Rand* p = op->CreateOpeartor(1);
number[0] = p->GetResult(); //数字
number[1] = p->GetResult(); 
number[2] = p->GetResult(); 
number[3] = p->GetResult();
sign[1] = '+';     
sign[2] = '-'; 
sign[3] = '*'; 
sign[4] = '/';
p = op->CreateOpeartor(2);
sign_1 = p->GetResult();//符号
sign_2 = p->GetResult();
sign_3 = p->GetResult();
p = op->CreateOpeartor(3);
bracket = p->GetResult();;//括号
}
